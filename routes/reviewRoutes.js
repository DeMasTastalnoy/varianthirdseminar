...
router.get("/:route_id", async (req, res) => {
    ...
    try {
        const result = await pool.query("SELECT reviews.comment, reviews.rating, users.name FROM reviews JOIN users ON reviews.user_id = users.id WHERE route_id = $1",[route_id]);
